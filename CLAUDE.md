# Claude Code Project Instructions — constant-time-eq-kotlin

## Project Overview

This is **constant-time-eq-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`constant_time_eq`](https://crates.io/crates/constant_time_eq).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/constant_time_eq/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/constant_time_eq>
